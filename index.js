...
//user_input.focus();
//user_input.addEventListener("blur", user_input.focus);
...
var text = "";
var started = false;
var state = 0; // 0 = not started, 1 = started, 2 = ended
...
function countdown()
{
    if (seconds == 0) // 60 seconds are up
    {
        ...
        //
        words.textContent = "enter to try again";
    }
    ...
    //     break;
    // timer.innerText = Number.toString(parseInt(timer.innerText)--);
}
...
function generateText()
{
    ...
    // realistically the user won't type more than 300 words in a minute
    for (let i = 0; i < 300; i++)
    {
        text += word_bank[Math.floor(Math.random() * 1000)] + " ";
    }
...
function typing(event)
{
    switch (state)
    {
        ...
        case 1: // started
            var user_char = String.fromCharCode(event.keyCode); // user's input
    
            // if the user types the correct character
            if (user_char == text[text_index])
            {
                while (wrong_count != 0) // get rid of wrong chars
                {
                    display.removeChild(display.lastChild);
                    wrong_count--;
                }
                text_index++; // go to the next char
                display.appendChild(document.createTextNode(user_char));
                words.textContent = text.substring(text_index, text_index + char_count); // move the text box
            }
            else // mistyped
            {
                wrong_count++, total_wrong++;
                let wrong = document.createElement("span");
                wrong.setAttribute("class", "display-wrong");
                // wrong.innerHTML = "&nbsp;";
                wrong.innerHTML = ((user_char == ' ') ? "&nbsp;" : user_char);
                // wrong.innerHTML = user_char;
                // display.innerHTML += "<span class='display-wrong'>&nbsp;</span>";
                display.appendChild(wrong);
            }
            break;
